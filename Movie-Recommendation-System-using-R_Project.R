...
setwd("C:/Users/shyam/Desktop/Movie-Recommendation-System-using-R_Project-main/Movie-Recommendation-System-using-R_Project-main")  # Add your own path 
...
movie_rating_data <- merge(rating_data, movie_data, by = "movieId")

# Calculate average rating for each movie
average_ratings <- movie_rating_data %>%
  group_by(movieId) %>%
  summarise(avg_rating = mean(rating))

# Determine genres of movies that the user has rated highly
user_id <- 1  # Replace with the desired user ID
user_ratings <- movie_rating_data %>%
  filter(userId == user_id) %>%
  select(movieId, rating)

highly_rated_genres <- movie_data %>%
  inner_join(user_ratings, by = "movieId") %>%
  inner_join(average_ratings, by = "movieId") %>%
  filter(rating > avg_rating) %>%
  distinct(genres) %>%
  pull(genres)

# Find movies with similar genres and recommend highest rated movies
recommended_movies <- movie_data %>%
  filter(genres %in% highly_rated_genres) %>%
  inner_join(average_ratings, by = "movieId") %>%
  arrange(desc(avg_rating)) %>%
  head(10)

# Print recommended movies
cat("Recommended movies based on user's rated genres:\n")
...
user_genre <- readline(prompt = "Enter a movie genre (or 'exit' to quit): ")

while (tolower(user_genre) != "exit") {
  # Find movies of the specified genre
  recommended_movies <- movie_data %>%
    filter(grepl(tolower(user_genre), tolower(genres))) %>%
    left_join(
      movie_rating_data %>%
        group_by(movieId) %>%
        summarise(avg_rating = mean(rating)),
  ...
  if (nrow(recommended_movies) == 0) {
    cat("No movies found for the specified genre.\n")
  } else {
    # Print recommended movies
    cat("\nRecommended movies for the specified genre:\n")
    print(recommended_movies)
  }
...
cat("Exiting the recommendation system.\n")
